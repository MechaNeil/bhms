...
DROP TABLE IF EXISTS SMS, Activity_Log, Suggestion, Payment_List, Payment_Invoice, Proof_of_Transaction, Payment_Method, Invoice, Bed_Assignment, Bed, Room, Property, Constant_Utility_Bills, Utility_Bills, Company, Role_Permission, Permission, Role, Status, Gender, Tenant, User;
...
CREATE TABLE Proof_of_Transaction (
    id INT PRIMARY KEY AUTO_INCREMENT,
    tenant_id INT, -- Foreign Key to Tenant
    payment_scope ENUM('tenant', 'room', 'partial_room') DEFAULT 'tenant',
...
-- Payment_Invoice Table (New Junction Table)
CREATE TABLE Payment_Invoice (
    id INT PRIMARY KEY AUTO_INCREMENT,
    proof_of_transaction_id INT, -- Foreign Key to Proof_of_Transaction
    invoice_id INT,             -- Foreign Key to Invoice
    amount_covered DECIMAL(10, 2),
    FOREIGN KEY (proof_of_transaction_id) REFERENCES Proof_of_Transaction(id),
