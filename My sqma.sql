...
CREATE TABLE Tenant (
    id INT PRIMARY KEY AUTO_INCREMENT,
    user_id INT UNIQUE NOT NULL,             -- Foreign Key to User
...
CREATE TABLE Room (
    ...
    FOREIGN KEY (property_id) REFERENCES Property(id),
    UNIQUE (property_id, room_no)            -- Ensures room numbers are unique per property
);
...
CREATE TABLE Bed (
    ...
    FOREIGN KEY (status_id) REFERENCES Status(id),
    UNIQUE (room_id, bed_no)                 -- Ensures bed numbers are unique per room
);
